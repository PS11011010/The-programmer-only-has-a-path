...
class TestSumOfElementsThatAreMultiplesOfK(unittest.TestCase):
  ...
  def test_zero_k(self):
    self.assertIsNone(sum_of_elements_that_are_multiples_of_k([1, 2, 3], 0), 'При нулевом параметре K вернулся непустой результат')

  def test_empty_arr(self):
    self.assertIsNone(sum_of_elements_that_are_multiples_of_k([], 42), 'При пустом параметре arr вернулся непустой результат')
